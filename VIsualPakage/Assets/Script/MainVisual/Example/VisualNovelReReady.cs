...
public class VisualNovelReReady : MonoBehaviour
{
    ...
    //캐릭터 정수(최대 3명)
    [HideInInspector]
    public int[] currentCharIndex = new int[3];
    [HideInInspector]
    //현재 말하고 있는 캐릭터 정수
    public int currentMentCharIndex = 0;
    //배경 정수 아이디
    [HideInInspector]
    public int currentBackNum;
    //타이핑 효과에서 쓰일 타이핑 속도
    [HideInInspector]
    public float typingSpd = 0.06f;
    //타이핑이 현재 진행중인가?
    [HideInInspector]
    ...
    //{
    //    GameObject newEmotion = Instantiate(useCharacter[currentDialogueIndex].spriteRenderers[val].gameObject);
    //    newEmotion.GetComponent<SpriteRenderer>().DOFade(1f, fadeTime / 10);
    //}
